...
const AdminPanel = () => {
  ...
  const [maintenanceMode, setMaintenanceMode] = useState(JSON.parse(localStorage.getItem('maintenance')) || false);
  ...
  const handleLogin = async (e) => {
    ...
    // Check lockout
    if (lockoutTime) {
      toast.error(`Account locked. Try again in ${Math.ceil((lockoutTime - Date.now()) / 1000)} seconds.`);
      setIsLoading(false);
      return;
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-gray-100">
        ...
        >
          ...
          <button 
            type="submit" 
            className={`w-full py-3 px-4 rounded-lg font-medium text-white transition-all ${isLoading || lockoutTime ? 'bg-gray-400 cursor-not-allowed' : 'bg-primary hover:bg-primary-dark'}`}
          ...
          {lockoutTime && (
            <p className="mt-3 text-sm text-center text-red-500">
              Try again in {Math.ceil((lockoutTime - Date.now()) / 1000)} seconds
            </p>
          )}
        </motion.form>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="border-b border-gray-200 mb-6">
          <nav className="-mb-px flex space-x-8">
            <button
              ...
              className={`py-4 px-1 border-b-2 font-medium text-sm flex items-center ${activeTab === 'updates' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
            >
            ...
            <button
              ...
              className={`py-4 px-1 border-b-2 font-medium text-sm flex items-center ${activeTab === 'settings' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
            >
        ...
        {activeTab === 'updates' && (
          ...
          >
            ...
            <div className="mb-6">
              <label htmlFor="updateText" className="block text-sm font-medium text-gray-700 mb-1">
                New Update
              </label>
              <input
                type="text"
              ...
              <div className="flex items-center mb-4">
                <label htmlFor="updateDays" className="block text-sm font-medium text-gray-700 mr-3">
                  Expires in:
                </label>
                <input
                  type="number"
            ...
            <div className="border-t border-gray-200 pt-4">
              ...
              {updates.length === 0 ? (
                ...
                <ul className="space-y-3">
                  {updates.map((u) => (
                    <li key={u.id} className="bg-gray-50 p-4 rounded-lg shadow-sm">
                      {editUpdateId === u.id ? (
          ...
          >
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className="space-y-4">
                <p className="text-sm text-gray-600">Change the visual appearance of the website</p>
                
                <div className="grid grid-cols-3 gap-3">
                  <button 
                    onClick={() => handleThemeChange('default')} 
                    className={`p-4 rounded-lg border-2 ${theme === 'default' ? 'border-primary bg-primary bg-opacity-10' : 'border-gray-200 hover:border-gray-300'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`p-4 rounded-lg border-2 ${theme === 'blue' ? 'border-primary bg-primary bg-opacity-10' : 'border-gray-200 hover:border-gray-300'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`p-4 rounded-lg border-2 ${theme === 'green' ? 'border-primary bg-primary bg-opacity-10' : 'border-gray-200 hover:border-gray-300'}`}
                  >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${maintenanceMode ? 'bg-red-500' : 'bg-gray-200'}`}
                  >
